...
- P3: a Supabase URL/ANON kulcs `--dart-define`-ból jőjjön, nincs kötelező `.env`, a hiányzó konfiguráció offline módot küld, és a docok, lokalizációk ennek megfelelően írják le.
...
* `app/lib/main.dart` → `const` compile-time Supabase konstansok, fallback `.env` load csak opcionális (debug), supabase guard provider override, `app/pubspec.yaml`-ból `flutter_dotenv` eltávolítható, `app/pubspec.lock` frissül.
...
* `flutter analyze`
* `flutter test`
* `flutter devices` → Android fizikai eszköz ID
* `flutter test integration_test -d <deviceId>` → offline/config statikus smoke
