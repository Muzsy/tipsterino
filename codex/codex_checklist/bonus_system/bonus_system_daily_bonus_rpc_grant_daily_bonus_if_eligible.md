...
- [x] A `supabase/sql_checks/bonus_system_rpc_daily_bonus_negative_access_checks.sql` kiterjesztve ellenőrzi az `authenticated` DML jogait a `reward_grants` és `user_stats` táblákon is, így a ledger szerződés teljesen védett.
