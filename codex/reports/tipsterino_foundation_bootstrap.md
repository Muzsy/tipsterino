...
- A Supabase inicializáció crashelt, ha a `.env` nem volt jelen, ezért az offline futtatás külön guard-ot igényelt.

## Mit módosítottunk?
- `app/lib/main.dart` → `dotenv` load + Supabase init only when configured, offline fallback, `ProviderScope` override.
...
- `.env.example`, `documents/app_architecture.md`, `codex/` checklist + report, `canvases/tipsterino_foundation_bootstrap.md`, `integration_test` + `test/widget` tesztjei; `sign_in_with_apple` verziófrissítés Supabase 2.12 mellett.
...
- Supabase login és register csak akkor működik teljesen, ha a `app/.env` fájlban helyes `SUPABASE_URL`/`SUPABASE_ANON_KEY` van.
...
1. Éles Supabase backend csatlakoztatása + `app/.env` környezeti fájl biztosítása (váltás env var vs secrets manager).
