\echo 'bonus_system RPC signup bonus behavior checks'
...
BEGIN
  ...
  END IF;
  IF (v_result->>'amount')::int != 100 THEN
  ...
  WHERE user_id = v_user_id AND code = 'signup_bonus';
  ...
  IF v_user_stats_tippcoins != 100 THEN
  ...
  END IF;
  IF v_result->>'reason' IS DISTINCT FROM 'already_granted' THEN
...
\echo 'bonus_system RPC signup bonus behavior checks completed'
