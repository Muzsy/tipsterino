\echo 'bonus_system RPC daily bonus negative access checks'
...
DO $$
BEGIN
  IF has_function_privilege('anon', 'public.grant_daily_bonus_if_eligible()', 'EXECUTE') THEN
    RAISE EXCEPTION 'anon has EXECUTE privilege on grant_daily_bonus_if_eligible()';
  END IF;
...
\echo 'bonus_system RPC daily bonus negative access checks completed'
