...
class FakePollingRepository extends UserEventsRepository {
  FakePollingRepository(this._events)
      : super(
          SupabaseClient(
            'http://localhost',
            'anon',
...
Widget _buildTestApp(FakePollingRepository repo) {
...
void main() {
  ...
  testWidgets('polling refresh triggers while visible and cancels on navigate away', (tester) async {
    final repo = FakePollingRepository([_buildEvent('event-1')]);
    ...
    await tester.pump(pollingDuration);
    final before = repo.fetchOffsets.length;
    await tester.pump(pollingDuration);
    expect(repo.fetchOffsets.length, equals(before));
  });
}
